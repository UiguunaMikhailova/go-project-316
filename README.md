...
| --- | --- | --- |
| `--depth` | `10` | количество уровней обхода |
...
Стартовая страница имеет `depth: 0`, страницы по ссылкам с неё — `depth: 1`,
и так далее. Флаг `--depth` задаёт количество уровней, которые попадут в отчёт:

| `--depth` | Что обходится |
| --- | --- |
| `1` | только стартовая страница |
| `2` | стартовая страница и страницы по её ссылкам |
| `3` | плюс ещё один уровень ссылок |

```bash
bin/hexlet-go-crawler --depth 3 https://example.com
```

### Повторные попытки

`--retries` задаёт, сколько раз запрос будет повторён сверх первой попытки:
`--retries 2` — это максимум три обращения к адресу, `--retries 0` — ровно одно.

### Скорость обхода

По умолчанию краулер не замедляется. Ограничить частоту запросов можно двумя
способами:

```bash
bin/hexlet-go-crawler --delay 200ms https://example.com   # пауза между запросами
bin/hexlet-go-crawler --rps 5 https://example.com         # не больше 5 запросов в секунду
```

`--delay` принимает длительность в формате Go: `200ms`, `1s`, `1500ms`, `2m`.
`--rps` принимает дробное число и пересчитывается в паузу `1s / rps`, то есть
`--rps 5` равносилен `--delay 200ms`. Если указаны оба флага, приоритет у
`--rps`.

Ограничение действует на весь процесс, а не на отдельный поток: пауза
выдерживается между любыми соседними HTTP-запросами — и загрузками страниц, и
проверками ссылок, и повторными попытками. Отмена (`Ctrl+C`) прерывает ожидание
сразу, отчёт формируется из уже собранных страниц.

В библиотеке за скорость отвечает поле `Options.Delay` типа `time.Duration`:
нулевое значение означает обход без ограничений, а пересчёт `--rps` в паузу
делает CLI.
